...
var Search = function() {
  ...
  */
  self.initApp = function() {
    ...
    */
    ...
    initMap();
    initWikiMedia();
    initFoursquare(self.currentSearch.resultsLimit);
  };
  ...
  self.reloadLocation = function(location) {
    ...
    if(location.locationId !== self.history()[self.currentLocation].locationId) {
      ...
      if(self.history()[self.currentLocation].storedCategories.length <= 0) {
        ...
        for(var group in location.storedPoints) {
      ...
      for(var group in location.storedPoints) {
        ...
        var pointsInGroup = location.storedPoints[group].length
        ...
        if(categoryGroup.id === 'all') {
          ...
          for(group in self.history()[self.currentLocation].storedPoints) {
            ...
            if(group !== self.appFilters.lastCategorySelected) {
              ...
              for(var i = 0; i < self.history()[self.currentLocation].storedPoints[group].length; i++) { //TODO calcular fuera history length
  ...
  *
  * @method
  * @param {int} resultsLimit - Limit used to set how many results from Foursquare we want to receive
  */
  function initFoursquare(resultsLimit) {
